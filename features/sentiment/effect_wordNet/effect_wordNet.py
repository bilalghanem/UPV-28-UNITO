...
class effect_wordNet_class:
    ...
    def score(self, sentence):
        ...
        pos = len(set(words).intersection(set(self.neg[self.neg['polar'] == '+Effect']['word'].tolist())))
        neg = len(set(words).intersection(set(self.neg[self.neg['polar'] == '-Effect']['word'].tolist())))
        return [pos, neg]

if __name__ == '__main__':
    s = ['love', 'hate', 'shit', 'hit']
    h = effect_wordNet_class()
    print(h.score('we are beautiful but misfire catch'))
